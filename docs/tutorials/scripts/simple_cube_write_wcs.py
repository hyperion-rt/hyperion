...
wcs.wcs.crpix = [image.flux.shape[1] / 2. + 0.5,
                 image.flux.shape[0] / 2. + 0.5]

# Set the coordinates of the image center
wcs.wcs.crval = [233.4452, 1.2233]

# Set the pixel scale (in deg/pix)
scale = np.degrees(3. * pc / image.flux.shape[0] / image.distance)
...
fits.writeto('simple_cube_slice_wcs.fits', image.flux[:, :, 1],
