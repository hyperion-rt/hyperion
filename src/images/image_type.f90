...
contains
  ...
  subroutine find_image_bin(img,x_image,y_image,ix,iy,ir)

    implicit none

    type(image),intent(in)    :: img  ! Image
    real(dp),intent(in) :: x_image, y_image
    integer,intent(out) :: ix,iy,ir
    real(dp) :: log10_r_image

    ! --- Now we have to figure out which pixel value to go to --- !

    log10_r_image = log10(sqrt(x_image*x_image + y_image*y_image))

    ! --- Find what bin this corresponds to --- !
  ...
  subroutine image_bin(img,p,x_image,y_image,im)
    ...
    ! Find image bin
    call find_image_bin(img,x_image,y_image,ix,iy,ir)
